...
<style>
  ...
  /* ── Force Streamlit's native toggle to always be visible, fixed to left edge ── */
  [data-testid="collapsedControl"] {
    ...
    transition: left 0.3s ease, width 0.2s !important;
  }
  [data-testid="collapsedControl"]:hover {
    width: 28px !important;
    background: #2d2d4e !important;
  }
  /* The SVG arrow inside the native button */
  [data-testid="collapsedControl"] svg {
    color: #a0a0c0 !important;
    width: 14px !important;
    height: 14px !important;
  }
  /* When sidebar IS open, shift the toggle to sit at its right edge */
  [data-testid="stSidebar"][aria-expanded="true"] ~ * [data-testid="collapsedControl"],
  .st-emotion-cache-1gwvy71 [data-testid="collapsedControl"] {
    left: 0px !important;
  }
  ...
  .prospect-card { background: rgba(108,99,255,0.08); border: 1px solid rgba(108,99,255,0.18); border-radius: 12px; padding: 14px 16px; margin: 8px 0; }
  ...
  .chat-section-title { font-family: 'Syne', sans-serif; font-weight: 700; font-size: 1.05rem; color: #1a1a2e; margin-bottom: 16px; }
  ...
  .route-signal { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: 0.67rem; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; margin-bottom: 10px; }
  ...
  .stTextInput > div > div > input { border: 1.5px solid #e0d9ff !important; border-radius: 12px !important; font-family: 'DM Sans', sans-serif !important; font-size: 0.9rem !important; padding: 12px 16px !important; background: #fafafe !important; color: #1a1a2e !important; }
  ...
  .stButton > button:hover { transform: translateY(-1px) !important; box-shadow: 0 5px 18px rgba(108,99,255,0.45) !important; }
  ...
  .dash-section-title { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 1.1rem; color: #1a1a2e; margin: 28px 0 14px; padding-bottom: 8px; border-bottom: 2px solid #eeebff; display: flex; align-items: center; gap: 8px; }
  .routing-table { width: 100%; border-collapse: collapse; font-size: 0.84rem; margin-top: 4px; }
  .routing-table thead tr { background: #1a1a2e; color: #ffffff; }
  .routing-table thead th { padding: 10px 14px; text-align: left; font-family: 'Syne', sans-serif; font-weight: 700; font-size: 0.72rem; letter-spacing: 0.8px; text-transform: uppercase; }
  .routing-table thead th:first-child { border-radius: 8px 0 0 0; }
  .routing-table thead th:last-child  { border-radius: 0 8px 0 0; }
  .routing-table tbody tr { border-bottom: 1px solid #f0eeff; }
  ...
  .model-chip { display: inline-block; padding: 2px 9px; border-radius: 20px; font-size: 0.7rem; font-weight: 700; }
  ...
  .dash-note { margin-top: 32px; text-align: center; font-size: 0.75rem; color: #b0a8d8; letter-spacing: 0.8px; }
  .dash-note span { color: #6c63ff; font-weight: 700; }

  [data-testid="stMetric"] { background: linear-gradient(135deg, #f8f7ff, #f2f0ff); border: 1px solid #e5e0ff; border-radius: 14px; padding: 18px 20px !important; }
...
# ── CascadeFlow ────────────────────────────────────────────────────────────────
COMPLEX_KEYWORDS = {
    "strategy","negotiate","negotiation","negotiating","proposal","objection",
    "objections","close","closing","contract","legal","competitor","competitive",
    "pricing","discount","escalate","stakeholder","executive","renewal",
    "churn","risk","upsell","expansion","forecast",
}
ROUTING_TIERS = [
    {"model":"gemma2-9b-it",           "label":"gemma2-9b",    "chip_class":"chip-fast",  "max_words":40,          "cost":0.001,  "tier_name":"Fast"},
    {"model":"llama-3.1-8b-instant",   "label":"llama-3.1-8b", "chip_class":"chip-mid",   "max_words":120,         "cost":0.0004, "tier_name":"Balanced"},
    {"model":"llama-3.3-70b-versatile","label":"llama-3.3-70b","chip_class":"chip-heavy", "max_words":float("inf"),"cost":0.008,  "tier_name":"Heavy"},
]
BASELINE_COST = 0.008
...
def route_query(text):
    ...
    if matched_kws:
        ...
        return {**tier,"words":word_count,"trigger":"keyword","matched_kws":matched_kws[:3],
                "reason":f"Keyword escalation: {', '.join(matched_kws[:3])}"}
    for tier in ROUTING_TIERS:
        if word_count <= tier["max_words"]:
            return {**tier,"words":word_count,"trigger":"length","matched_kws":[],
                    "reason":f"{tier['tier_name']} tier — {word_count} words"}
    tier = ROUTING_TIERS[-1]
    return {**tier,"words":word_count,"trigger":"length","matched_kws":[],"reason":f"Long query — {word_count} words"}
...
if "selected_prospect_id" not in st.session_state: st.session_state.selected_prospect_id = None
if "chat_histories"       not in st.session_state: st.session_state.chat_histories = {}
if "query_log"            not in st.session_state: st.session_state.query_log = []

def get_prospect(pid): return next((p for p in PROSPECTS if p["id"]==pid),None)

def call_groq(messages,model):
    api_key = os.getenv("GROQ_API_KEY")
    if not api_key: return "⚠️ GROQ_API_KEY not found."
    try:
        client = Groq(api_key=api_key)
        r = client.chat.completions.create(model=model,messages=messages,max_tokens=1024,temperature=0.7)
...
Deal Type: {p['deal_type']} | Deal Size: {p['deal_size']} | Stage: {p['stage']}
Last Contact: {p['last_contact']} | Next Step: {p['next_step']}
Notes: {p['notes']}

Be concise, commercially sharp, and reference this prospect's specific context in every response.
Format key recommendations with a ▸ prefix."""
...
with st.sidebar:
    ...
    <div class="brand-header">
      ...
      <div><div class="brand-name">DealMind</div><div class="brand-tag">Sales Intelligence</div></div>
...
if prospect_id is None:
    ...
    with tab_coach:
        ...
        <div class="profile-card">
          ...
          <div class="profile-stats">
            <div class="stat-item"><span class="stat-label">Deal Size</span><span class="stat-value deal-size">{prospect['deal_size']}</span></div>
            <div class="stat-item"><span class="stat-label">Type</span><span class="stat-value">{prospect['deal_type']}</span></div>
            <div class="stat-item"><span class="stat-label">Last Contact</span><span class="stat-value">{prospect['last_contact']}</span></div>
            <div class="stat-item"><span class="stat-label">Next Step</span><span class="stat-value">{prospect['next_step']}</span></div>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        if not chat_history:
            ...
            <div class="chat-message-wrap ai">
              ...
              <div class="chat-bubble-ai">
                I'm locked in on the <strong>{prospect['deal_size']} {prospect['deal_type']}</strong> deal with
                <strong>{prospect['name']}</strong> at {prospect['company']}.<br><br>
                They're in <strong>{prospect['stage']}</strong> — next step: <em>{prospect['next_step']}</em>.<br><br>
        ...
        for msg in chat_history:
            if msg["role"] == "user":
                ...
                if msg.get("route_trigger") == "keyword" and msg.get("route_kws"):
                    signal_html = f'<div class="route-signal keyword"><div class="route-dot"></div>Keyword escalation · {", ".join(msg["route_kws"])} · {msg.get("route_model","")}</div>'
        ...
        with col1:
            user_input = st.text_input("Message", placeholder=f"Ask about the {prospect['name']} deal…",
                                       label_visibility="collapsed", key=f"input_{prospect_id}")
        with col2:
        ...
        def handle_send(text):
            ...
            api_msgs = [{"role":"system","content":build_system_prompt(prospect)}] + [
                {"role":m["role"],"content":m["content"]} for m in chat_history]
            with st.spinner("DealMind is thinking…"):
                reply = call_groq(api_msgs, tier["model"])
            chat_history.append({"role":"assistant","content":reply,
                "route_trigger":tier["trigger"],"route_kws":tier["matched_kws"],
                "route_words":tier["words"],"route_model":tier["label"]})
            st.session_state.chat_histories[prospect_id] = chat_history
            st.session_state.query_log.append({
                "num":len(st.session_state.query_log)+1,"words":tier["words"],
                "model":tier["label"],"chip_class":tier["chip_class"],
                "reason":tier["reason"],"trigger":tier["trigger"],
                "matched_kws":tier["matched_kws"],"cost":tier["cost"]})
            st.rerun()
    ...
    with tab_dash:
        ...
        kw_esc        = sum(1 for r in log if r.get("trigger")=="keyword")
        len_routed    = total_queries-kw_esc

        m1,m2,m3,m4 = st.columns(4)
        with m1: st.metric("Total Queries", total_queries)
        with m2: st.metric("Avg Cost / Query", f"${avg_cost:.4f}")
        with m3: st.metric("Est. Without Routing", f"${est_no_route:.4f}")
        with m4: st.metric("Money Saved", f"${money_saved:.4f}", delta=f"{pct_saved:.1f}% saved" if total_queries else "—")

        if total_queries:
            st.markdown('<div class="dash-section-title">🔀 Signal Breakdown</div>', unsafe_allow_html=True)
            sb1,sb2 = st.columns(2)
            with sb1: st.metric("Keyword Escalations 🔑", kw_esc, delta=f"{kw_esc/total_queries*100:.0f}% of queries")
            with sb2: st.metric("Length-Routed ↔", len_routed, delta=f"{len_routed/total_queries*100:.0f}% of queries")

        st.markdown('<div class="dash-section-title">📋 Routing Decisions</div>', unsafe_allow_html=True)
        if not log:
            st.markdown('<div style="text-align:center;padding:40px;color:#b0a8d8;font-size:0.9rem;">No queries yet — send a message in the Deal Coach tab.</div>', unsafe_allow_html=True)
        else:
            rows = ""
            for r in log:
                tc = (f'<span class="trigger-kw">🔑 {", ".join(r["matched_kws"])}</span>'
                      if r.get("trigger")=="keyword" and r.get("matched_kws")
                      else f'<span class="trigger-len">↔ {r["words"]} words</span>')
                rows += f"<tr><td>#{r['num']}</td><td>{tc}</td><td><span class='model-chip {r['chip_class']}'>{r['model']}</span></td><td>{r['reason']}</td><td class='cost-cell'>${r['cost']:.4f}</td></tr>"
            st.markdown(f"""<table class="routing-table"><thead><tr>
              <th>Query #</th><th>Signal</th><th>Model</th><th>Reason</th><th>Cost</th>
            </tr></thead><tbody>{rows}</tbody></table>""", unsafe_allow_html=True)

            st.markdown('<div class="dash-section-title">📊 Queries by Model</div>', unsafe_allow_html=True)
            mc = {}
            for r in log: mc[r["model"]] = mc.get(r["model"],0)+1
            st.bar_chart(pd.DataFrame({"Queries":mc}), color="#6c63ff")

            st.markdown('<div class="dash-section-title">💰 Cost by Model</div>', unsafe_allow_html=True)
            mco = {}
            for r in log: mco[r["model"]] = round(mco.get(r["model"],0.0)+r["cost"],6)
            st.bar_chart(pd.DataFrame({"Cost ($)":mco}), color="#a78bfa")

        st.markdown('<div class="dash-note">Powered by <span>CascadeFlow</span> — keyword escalation + length-based routing across 3 model tiers.</div>', unsafe_allow_html=True)